...
((x.Day7Type.HasValue && x.Day7Type.Value) ? 1 : 0);
            ...
            return shifts
                .Select(x => new ShiftDTO { ShiftId = x.ShiftId, Name1 = x.Name1, HourCount = x.HourCount, NumberOfDays = x.NumberOfDays }).ToList();
        }
...
where rowNum={index} 
";
            var data = _repShift.ExecuteStoredProcedure<HrShifts>
        ...
        public int GetCount()
          => _repShift.Find(x => !x.DeletedAt.HasValue).Count();
